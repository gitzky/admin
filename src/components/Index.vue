    ...
    <section class="sec-right">
      ...
      <div class="top-c">
        <header>
          <div class="h-left">
            <div class="pointer" @click="isShrinkAside" title="收缩/展开">
              <Icon
                ...
                class="fb"
                size="30"
                :class="isOpen?'rotated90_0':'rotated0_90'"
              />
            </div>
            <!-- 面包屑功能 -->
          ...
          <div class="h-right">
            <div class="notice-t">
              <span class="home-time mar20_lr">
                <Icon class="mar5_lr" size="28px" type="md-alarm" />
                <span class="vertical_m fb">{{time}}</span>
              </span>
            </div>
            <!-- 消息 -->
            ...
            <el-drawer title="消息标题" direction="rtl" :visible.sync="showMessage">
              <span class="block pad20">我来啦!</span>
            </el-drawer>
            <!-- 用户头像 -->
...
export default {
  ...
  created() {
    ...
    setInterval(() => {
      let time = new Date();
      time = moment(time).format("YYYY 年 MM 月 DD 日   HH : mm : ss");
      this.time = time;
    }, 1000);
  },
  mounted() {
  ...
  computed: {
    // 菜单栏
    menuItems() {
      return this.$store.state.index.menuItems;
    },
    // 需要缓存的路由
    keepAliveData() {
